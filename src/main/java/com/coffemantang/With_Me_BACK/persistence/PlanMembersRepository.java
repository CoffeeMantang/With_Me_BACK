...
public interface PlanMembersRepository extends JpaRepository<PlanMembers, Integer> {
    ...
    @Query(value = "UPDATE plan p INNER JOIN plan_members pm ON p.plan_id = pm.plan_id SET pm.`check` = 1 WHERE p.state = 3 AND pm.member_id = :memberId and pm.`check` = 0", nativeQuery = true)
    void updateCheck1(@Param("memberId") int memberId);

    // planId, memberId에 맞는 로우의 check 수정
    @Query(value = "UPDATE plan_members SET `check` = :check WHERE plan_id = :planId AND member_id = :memberId", nativeQuery = true)
    void updateCheck2(@Param("planId") int planId, @Param("memberId") int memberId, @Param("check") int check);
}
